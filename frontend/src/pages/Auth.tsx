...
export default function Auth() {
  ...
  const { isAuthenticating, signUpWithEmail, signInWithEmail, signInWithGoogle, user } = useAuth();
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (session?.user) {
        await supabase
          .from("profiles")
          .upsert({ id: session.user.id, full_name: fullName, email }, { onConflict: "id" });
      }
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center px-12 text-white">
          ...
          <p className="text-xl text-white/90">
            AI-powered college planning that understands your unique goals and preferences.
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 gradient-subtle">
        <Card className="w-full max-w-md p-8 shadow-card-hover">
          <div className="flex items-center gap-2 mb-8 lg:hidden">
            ...
            <h2 className="text-2xl font-bold">PathFinder AI</h2>
          </div>

          <Tabs defaultValue={user ? "login" : "signup"} className="w-full">
          ...
          <div className="mt-6">
            ...
            >
              ...
              {!isSubmitting && <span>Sign in with Google</span>}
            </Button>
          </div>
        </Card>
      </div>
